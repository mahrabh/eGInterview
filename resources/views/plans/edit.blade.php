            ...
            <aside class="lg:col-span-4">
                <div class="glass-panel rounded-xl overflow-hidden h-full">
                    <div class="px-4 py-3 bg-gradient-to-br from-slate-50 via-indigo-50/50 to-cyan-50/40 border-b border-slate-200/80">
                        <div class="flex items-center gap-2.5">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-[11px] text-slate-500 font-mono truncate">{{ $plan->slug }}</p>
                            </div>
                        </div>
                        <div class="mt-2 flex flex-wrap gap-1">
                            <span class="inline-flex px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide rounded-full {{ $plan->is_active ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-500 border border-slate-200' }}">
                    ...
                    <div class="p-3.5 space-y-2">
                        <p class="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Current limits</p>
                        <div class="flex items-center justify-between gap-2 text-xs text-slate-600">
                            <span>Interviews</span>
                            <span class="font-semibold text-slate-900 tabular-nums">{{ number_format($plan->interview_limit) }}</span>
                        </div>
                        <div class="flex items-center justify-between gap-2 text-xs text-slate-600">
                            <span>AI generations</span>
                            <span class="font-semibold text-slate-900 tabular-nums">{{ number_format($plan->ai_generation_limit) }}</span>
                        </div>
                    </div>
                </div>
            </aside>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            <div>
                                ...
                                <input id="slug" type="text" name="slug" value="{{ old('slug', $plan->slug) }}" required
                                    class="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-900 font-mono focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all">
